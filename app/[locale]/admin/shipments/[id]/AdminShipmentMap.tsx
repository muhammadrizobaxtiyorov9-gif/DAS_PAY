...
export default function AdminShipmentMap({ 
  ...
  let durationHours = 0;
  let durationMins = 0;
  let stopDurationMins = 0;
  const points: [number, number][] = hasLogs ? logs.map((l: LogEntry) => [l.lat, l.lng]) : [];
  const stopPoints: [number, number][] = hasLogs ? logs.filter((l: LogEntry) => l.isStop).map((l: LogEntry) => [l.lat, l.lng]) : [];
  const stopLogs = hasLogs ? logs.filter(l => l.isStop) : [];
  const start = hasLogs ? logs[0] : null;
  const end = hasLogs ? logs[logs.length - 1] : null;
  ...
  if (hasLogs) {
    ...
    durationMins = Math.floor((durationMs % (1000 * 60 * 60)) / (1000 * 60));
    stopDurationMins = Math.round(stops * 0.5);
  }
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-blue-50/30">
        <h3 className="font-bold text-gray-800 flex items-center gap-2 text-base">
          ...
          {transportMode === 'truck' ? 'Avtomobil marshruti va GPS Tracking' : 'Temir yo\'l marshruti'}
        </h3>
      ...
      {/* Detailed Stats Grid - only when GPS logs exist */}
      {hasLogs && (
        <>
          <div className="p-4 bg-slate-50 border-t border-gray-100 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ...
            <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
              ...
              <div className="text-sm font-bold text-slate-800">{start ? new Date(start.createdAt).toLocaleString('uz-UZ', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}</div>
            </div>
          </div>
